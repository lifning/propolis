...
impl VncServer {
    ...
    pub fn start() {
        ...
        //info!(self.log, "vnc-server listening on port {}", self.port);
        println!("vnc-server listening on port {}", 5900);

        std::thread::spawn(move || {
            for stream in listener.incoming() {
                VncServer::handle_vnc_client(stream.unwrap());
            }
        });
    }
    ...
    fn handle_vnc_client(mut stream: TcpStream) {
        ...
        stream
            .write_all(&sec_options)
            .expect("could not write security handshake");
